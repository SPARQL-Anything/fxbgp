...
public class AnalyserCSVTest extends BGPTestAbstract {
	...
	public AnalyserCSVTest() {
		super(FXCSVModel.getFXCSVModel());
	}
...
//
	...
	public void AT2() throws IOException {
		...
		//Assert.assertEquals(32,annotations().size());
		Assert.assertEquals(16,annotations().size());

		int A = 0;
		for(FXBGPAnnotation fi: annotations()){
			A++;
			L.info("AT2/{} -- {}",A, fi.toString());
			Assert.assertTrue(fi.isGrounded());
		}
	}
